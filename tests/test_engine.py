...
def test_opciones_desde_pausa_rehace_los_botones_si_cambia_el_idioma(juego, monkeypatch):
    """Los botones de pausa cachean su texto: tras cambiar de idioma en Opciones
    deben recrearse con el idioma nuevo (y no quedarse en el anterior)."""
    from src.core import i18n
    from src.ui.menu import MenuManager

    juego.pausar_juego()
    juego.dibujar()
    assert juego.boton_reanudar.texto == "Reanudar"

    def _cambia_a_ingles(self):
        i18n.establecer_idioma("en")
        return None

    monkeypatch.setattr(MenuManager, "mostrar_solo_opciones", _cambia_a_ingles)
    juego.mostrar_opciones_juego()
    juego.dibujar()
