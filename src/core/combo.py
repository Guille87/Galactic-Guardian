...
class Combo:
    ...
    def multiplicador(self):
        return 1 + sum(1 for umbral in settings.COMBO_UMBRALES if self.racha >= umbral)

    @property
    def es_maximo(self):
        return self.multiplicador == 1 + len(settings.COMBO_UMBRALES)
    ...
    def progreso(self):
        ...
        desde = settings.COMBO_UMBRALES[nivel - 1] if nivel else 0
        hasta = settings.COMBO_UMBRALES[nivel]
