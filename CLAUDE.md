...
Assets live under `data/assets/` (`imagenes/`, `musica/`, `sonidos/`). Logical asset names are mapped to relative paths in the `RECURSOS`, `SONIDOS`, and `EXPLOSIONES` dicts in `src/core/config.py` — add new assets there, not by hardcoding paths.
...
- **`RenderManager`** (`src/managers/render.py`) — orchestrates all per-frame drawing, including the greyed-out paused view and the game-over/name-entry overlays. It **creates the Game Over / pause buttons as a side effect of drawing** and stashes them on `juego` (`boton_reintentar`, `boton_salir_post`, `boton_opciones`, `boton_salir`); `InputHandler` reads those same attributes — so button hitboxes only exist after a render pass.
...
- **`AudioManager`** (`src/core/audio.py`) — music/effect playback and volume. **One instance**, built in `main.py` and injected into both `MenuManager` and `Juego`. Effect names used in code (`"disparo"`, `"golpe"`, `"item"`) are AudioManager-level aliases, distinct from the raw `SONIDOS` keys (`laser_gun`, `hit`, `item_take`). (Music is still played as `mixer.Sound`, not `mixer.music` — slated for Phase 2.)
...
`ResourceManager` (`src/core/resources.py`) is a `__new__`-based singleton; `get_image_scaled` caches scaled+`convert_alpha`'d surfaces by `name_WxH` key. It also keeps original file paths (`get_image_path`) because projectile classes load their own image from a path.

Enemies (`src/entities/enemies.py`): `EnemigoBase` → `EnemigoTipo1/2/3` and `Jefe`. Health scales as `salud_base * 2**(nivel-1)`. Loot probability rises by type (5% / 10% / 20% / 100% boss) and is forced after 10 kills; the drop pool is filtered by what the player still needs. `Jefe` overrides `movimiento_enemigo` (no-op) and drives its own patrol in `update`.

Projectiles: `src/entities/base/projectile_base.py` is the shared base for `Bullet` (`bullet.py`) and `BalaEnemigo` (`bullet_enemy.py`). Player upgrades (damage/cadence/speed, `tipo_disparo` up to `"triple"`) live on `Jugador` (`src/entities/player.py`); `Item` (`src/entities/items.py`) applies power-up effects on pickup.
