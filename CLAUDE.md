...
The `pygame_gui` options screen uses the 0.6+ event API (`event.type == pygame_gui.UI_BUTTON_PRESSED` / `UI_HORIZONTAL_SLIDER_MOVED`), not the old `USEREVENT` + `event.user_type`. `MenuManager` keeps a single `self.clock` (only the outer loop `tick`s) and a single `self.ui_manager`, built lazily once by `_inicializar_interfaz_opciones` and re-synced on each entry by `_abrir_opciones`. Volume handling for **both** sliders: dragging the bar is free (`UI_HORIZONTAL_SLIDER_MOVED` → `_clamp_volumen`, just `[0,1]` + round to 2 dp); the ◄ ► arrows (`UI_BUTTON_PRESSED`) jump to the multiple of `VOLUMEN_PASO` just above/below the current value (`_paso_volumen`) and mark the destination in `_ignorar_moved` so the `MOVED` `pygame_gui` fires right after is discarded. Every applied value is pushed back with `set_current_value` — a value outside `[0,1]`, even by `1e-17`, makes `pygame_gui` silently freeze the slider.
